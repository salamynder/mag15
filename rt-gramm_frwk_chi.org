...
#+BEGIN_LaTeX
\DTLsetseparator{;}
\DTLloadrawdb[noheader,keys={Api,Eng,Chi,Status,Comm}]{eval}{tests/eng_chi3.csv}
%% \DTLsort{Status=ascending}{eval}%
\begin{longtable}[l]{p{0.33\textwidth} p{0.25\textwidth} p{0.16\textwidth} p{0.08\textwidth} p{0.28\textwidth}}
...
\bfseries Kommentar \\
\DTLforeach*{eval}{
\api=Api,\eng=Eng,\chi=Chi,\stat=Status,\comm=Comm}{%
\tiny         \api &
\footnotesize \eng &
\footnotesize \chi &
\footnotesize \stat &
