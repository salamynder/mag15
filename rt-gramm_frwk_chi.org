...
| API-Baum-Element | engl. Linear. | chin. Linear. | Güte |
|------------------+---------------+---------------+-------|
...
Die Auswahl der 450 Bäume erhebt keinen Anspruch auf eine vollständige Darstellung der RGL, aber in jedem Fall lassen sich anhand dieser vielerlei syntaktische Strukturen auf (a) grammatische und (b) pragmatische Korrektheit (gängiges, gutes Chinesisch) hin prüfen. ~GOOD~ bescheinigt daher, dass sowohl (a) und (b) zutreffen; ~BAD~, dass (a) nicht zutrifft; und ~SOSO~, dass zwar (a) zutrifft, aber (b) nicht.[fn::\cite[33]{_implementation_2013}. Zusätzlich zu diesen Kriterien sollte natürlich auch immer die /inhaltlich/ korrekte Umsetzung des Funktionsbaums in die Linearisierung gewährleistet sein. So ist das Ergebnis (~她知道我们不看了的是谁~) aus Auflistung [[ec2_whom_does]] nur grammatisch korrekt, insofern man es als Negation einer abgeschlossenen Handlung in der Zukunft (~不看了~) deutet. Jedoch ist die Zukunft durch den Funktionsbaum in Zeile 5 nicht gemeint, sondern vielmehr die Vergangenheit (\inlst$pastTense anteriorAnt$), welche aber nicht mittels 不 sondern mit 没（有） verneint wird, was insgesamt die Einstufung in Güteklasse ~SOSO~ wegen inhaltlich-grammatischer Inkorrektheit als sehr optimistisch erscheinen lässt. Theoretisch hätte dieses Ergebnis also genauso gut als ~BAD~ klassifiziert werden können, praktisch gesehen ist dieses Dilemma aber einerlei, da uns im Zuge der Auswertung beide Arten von Mangelfällen besonders interessieren.]
...
** Auswertungsmethodik
Zur besseren Handhabbarkeit der einzelnen Einträge sowie der verschiedenen Kategorien innerhalb dieser wurde Chens Dokument in ein simples Tabellen-Dateiformat überführt.[fn:: Zu finden unter https://github.com/salamynder/mag15/blob/master/tests/eng_chi3.csv .]

Aufgabe meinerseits war es nun, Chens Liste erneut durchzusehen, um zu klären, welche chinesischen C
- 
- eine Fehler-Kategorisierung innerhalb dieser Liste vorzunehmDie chinesische Linearsierung ließ sich mittels des Quellcode-Moduls ~api/TryChi.gf~ bzw. des kompilierten Moduls ~alltenses/TryChi.gfo~ reproduzieren, wobei hier auch schon Abweichungen, wie in den eben angeführten Beispiel, zu beobachten waren:
#+BEGIN_SRC empty
...
Die 是-的-Konstruktion bietet eine Möglichkeit, bestimmte Satzglieder zu betonen, und erfüllt daher dieselbe Funktion wie Spaltsätze im Deutschen oder Englischen. \xpinyin*{是} für sich genommen ist aber auch einfach das Vollverb »sein«, das für Klassifikation und Äquation zuständig ist. In den folgenden Daten wird genauer auf diese Parallelen und Funktionen eingegangen.

Diese Kategorie lässt sich zunächst grob unterteilen in Sätze, in denen der Einsatz von 是-（的） entweder unbedingt vonnöten oder aber auch sehr angeraten erscheint, und solche, in denen die Verwendung schlichtweg grammatisch falsch oder eine pragmatisch unsachgemäße Betonung hervorgerufen wird.
...
{\DTLforeach*{\shideg}
  ...
  \todo=Field6}
    {%
    \scriptsize\raggedright \api \par &
    \scriptsize \eng &
    \scriptsize \chi &
    \scriptsize \stat &
...
\end{longtable}
#+END_LaTeX
Man beachte, dass die ersten drei Zeilen der obigen Tabelle die angestrebte Korrektur schon zeitigen, d.h. 是-的 wird hier standardmäßig nicht mehr benutzt, was sich anhand der Chinesisch-Spalte zeigt, in der geteilt durch einen Pfeil (~->~) ein älteres und ein aktuelles Linearisierungsergebnis dokumentiert ist.[fn:: Diese positive Veränderung ist dokumentiert in Commit ba6bd33b vom 08.10.2013 und betrifft vor allem die Funktion ~QuestSlash~ aus \inlst$chinese/QuestionChi.gf$, die aufgerufen wird, wenn ~mkQCl~ als zweites Argument eines vom Typ ~ClSlash~ -- also einem Satz (Clause), der ohne sein Subjekt (~NP~) prädiziert wird -- nimmt. REFTODO:Explanation:Slash!]

In den restlichen Zeilen sieht man an der Chinesisch-Linearisierung, dass hier noch kein Paradigma vorhanden ist, welches auf die Konstruktion mit 是-的 verzichten würde. Vielmehr zeigt sich, dass 是 als Äquationalkopula in allen Fällen (mkCl : NP -> A/AP -> Cl) Anwendung findet, was vor allem in den Zeilen 5 bis 10 problematisch ist, da hier über die Äquation keine besondere Betonung realisiert wird. Zum Beispiel ist in der Aussage von Zeile 8 »she is old« grammatisch völlig unbetont, weswegen das chinesische Äquivalent eher heißen sollte 「她很老」 anstatt 「她是老的」.[fn:: Für die Unterscheidung zwischen der Bildung des Ajektivprädikats mittels 很 oder 是， siehe \cite[126f.]{motsch_grundlagen_2010}. A.a.O. ist auch bemerkenswert, dass die Klassifizierung als alt in 「她是老的」 abwertend gemeint sein kann, wie in 姓王的， 不过是个教书的 -- wobei man hier einwenden mag, dass auch 个教书的 (»einer der aus Büchern lehrt«) schon keine Respektsbezeugung darstellt. Dennoch bliebe festzuhalten, dass hier 是 als Äquationskopula den formalen Rahmen für die Klassifikation durch Äquation schafft.] 

Umgekehrt scheint aber eine grammatische Betonung von »here« in Zeile 4 [(a) »it is here that she sleeps«] durchaus vorzuliegen, denn die Satzaussage könnte auch einfacher mit (b) »she sleeps here« gebildet werden, wobei die Auslagerung eines zu betonenden Satzgliedes in einen vorangestellten unpersönlichen Satz (»it is \ldots{}«) gemeinhin als Kluft- oder /Spaltsatz/ bekannt ist.[fn::REFTODO Hadumod] Mit der gegebenen grammatischen Betonung im Englischen sollte also auch das chinesische Äquivalent eine Hervorhebung des Ortes, also 这里/儿, genießen dürfen, woraus sich mindestens zwei Möglichkeiten ergeben:

- (1) 「她睡觉的是这里」\hspace{0.3em}(s. Zeile 4)
  - Hier wäre eine Voranstellung von "hier/这里" und sodann auch das Einfügen von 就 möglich: 「这（里）（就）是她睡觉的地方」
...
Dazu ist tentativ noch anzumerken, dass eine Konsultation unter Muttersprachlern erbrachte, dass eine Konstruktion wie in (2) schon als durchaus angemessen erachtet wird, um eine (wenn auch indirekte) Betonung des Ortes zu forcieren.
***** TODO eval[THINK: argument: for wide-coverage shi-de needed?]
Die jetzige (1--3) bzw. angestrebte (5--10) Linearisierung ohne 是-的 liefert gute Ergebnisse und ist tendenziell auch für den Fall aus Zeile 4 geeignet. Dennoch sollte die Möglichkeit, für alle Fälle eine 是-的-Variante zu erstellen, gegeben sein. (Wie sich dies in der RGL-API realisieren lässt, ist aber sehr unklar.) 
...
\end{longtable}
#+END_LaTeX
Fall Nr. 1 ist besonders interessant, weil wir es hier mit einem Spaltsatz zu tun haben, der ziemlich sicher ohne 是-的 auskommen muss, was mit dem Zustandsverb \xpinyin*{确定} (sicher sein) zusammenhängen könnte. Hier noch einmal der /gute/ Spaltsatz aus der letzten Sektion, der 是 benutzen darf, und Fall Nr. 1 im Vergleich:
#+BEGIN_SRC empty
mkUtt                              mkUtt
 (mkCl here_Adv                    (mkCl
       (mkS                         (mkVP
         (mkCl she_NP sleep_V)))     (mkAP
                                      (mkAP uncertain_A)
                                      (mkQS
                                       (mkQCl who_IP sleep_V)))))

it is here that she sleeps         it is uncertain who sleeps
她睡觉的是这里 G (...)              是不确定谁睡觉的 B 不确定谁睡
#+END_SRC

Was Fall Nr. 1 betrifft, so urteilt Chen also, dass die Linearisierung unzulässig ist. Auch eine Befragung unter Muttersprachlern ergab, dass das Ergebnis von 1 eher wie eine direkte Übersetzung aus westlichen Sprachen klingt bzw. als eine solche zu erkennen ist, und als hervorstechender Grund hierfür ließe sich anführen, dass der Adjektivphrase 「不确定」 mit 确定 als Zustandsverb schon zuviel »Sein« innewohnt, als dass 「不确定」 in irgendeiner Weise mit 是 -- sei es 「是不确定」 oder 「不是确定」 -- kombiniert werden dürfte. Adverbiale Bestimmungen, wie 这里 aus dem /guten/ Spaltsatz, scheinen sich jedenfalls eher, mit 是-（的） kombinieren zu lassen.[fn:: In diesem Zusammenhang darf auch daran erinnert werden, dass das eingefügte 很 bei der Adjektivprädizierung, wie etwa 「这间屋子很大」 （»dieses Zimmer ist groß«）, eigentlich keine Kopula ist, sondern ein Adverb (mit der Bedeutung »sehr«, die aber so gut wie verblasst ist, insofern man 很 nicht sehr stark betont ausspricht), dessen vorrangige Aufgabe es ist, das Zustandsverb 大 derart zu bestimmen, dass es auch wirklich »groß« bedeutet; nimmt man 很 nämlich weg, dann nimmt 大 den Komparativ an. Cf. \cite[239]{kneussel_grammatik_2005}]

Unabhängig von den postulierten Gründen für die Mängel am Ergebnis von Fall 1 können wir aber auch konstatieren, dass der Vorschlag 「不确定谁睡」 im Vergleich zur englischen Version genau die Elemente in der für das Chinesischen richtigen grammatischen Sequenz aufweist, die essentiell notwendig sind, um die Bedeutung von »it is uncertain who sleeps« zu bewahren, nämlich als Hauptsatz die Aussage »nicht sicher« sowie diesem untergeordnet die Frage »wer schläft«. Aus dieser Perspektive nicht essentiell sind also das unpersönliche Pronomen »it« sowie die unterordnende Konjunktion »that«; die Kopula-Form »is« bleibt hingegen essentiell, ist aber versteckt im Zustandsverb 确定. Daher ist der Vorschlag ein Spaltsatz.

Der simple englische Satz zu dem gespaltenen wäre übrigens »who sleeps is uncertain«, was den Sinn des gespaltenen aber nur ambig wiedergibt, denn er könnte bedeuten, dass die schlafenden Subjekte in sich selbst unsicher sind (»wer schläft, der ist auch unsicher«) weswegen ist der angesprochene Spaltsatz doch kein richtiger
...
#+LATEX_HEADER: %TODO: Bug? erste Zelle einer longtable (unter Header): baselinestretch zu hoch
#+LATEX_HEADER:  
#+LATEX_HEADER:
# Local Variables:
# zotero-collection: #("4" 0 1 (name "ChinGrammar"))
